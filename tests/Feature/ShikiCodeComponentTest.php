...
it('renders the component with syntax highlighting classes', function () {
    ...
    $rendered = Blade::render('<x-shiki-code language="php" :code="$code" />', ['code' => $code]);

    // Assert structure
    expect($rendered)->toContain('class="shiki-wrapper"')->toContain('class="language-php shiki-renderer"');
...
it('renders plain text for unknown languages', function () {
    ...
    $rendered = Blade::render('<x-shiki-code language="unknown" :code="$code" />', ['code' => $code]);
...
it('uses slot content when code prop is not provided', function () {
    ...
    expect($result)
        ->toContain('shiki-wrapper')
        ->and($result)
        ->toContain('language-php')
        ->and($result)
        ->toContain('12345'); // The cast number as string
});
...
it('requires string cast for non-string slot content', function () {
    ...
    expect($result1)->toContain('42')->and($result1)->toContain('shiki-wrapper');
...
it('escapes HTML in fallback when exception occurs', function () {
    ...
    $rendered = Blade::render('<x-shiki-code language="invalid-language-that-will-throw" :code="$code" />', [
        'code' => $code,
    ]);
